...
### Added
- `package_layout` — customizable package directory structure with source-glob-to-destination mapping, supporting `${ENV}` interpolation and glob patterns
- `post_install` hook — runs on each remote server after extract + symlink, before stop/start (e.g., create virtualenvs, `pip install`, database migrations)
- Install creates persistent symlinks — `current/log` → `{baseDir}/log` and `current/{data_dir}` → `{baseDir}/{data_dir}` automatically during install
- `tow doctor` server dependency checks — verifies required tools (`bash`, `tar`, `lsof`, `curl`) and recommended tools (`nc`) on remote servers, shows affected commands and install guidance

## [0.3.1] - 2026-04-02

### Fixed
- All version references synced across codebase
- Documentation uses major.minor (v0.3) for stability
- MCP server, VS Code extension, npm, Homebrew version aligned
